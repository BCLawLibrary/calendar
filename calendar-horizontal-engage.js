...
//sort events by start date
	xmlevents.sort(function(a,b){
     a = new Date(Date.parse($(a).find("event\\:start, start").text()));
     b = new Date(Date.parse($(b).find("event\\:start, start").text()));
     return a-b;
    });
  ...
  xmlevents.each(function(index,val){
  	...
  	if (filter =="All") {
  	 if (index > 3) {
     	...
     	var eventDate= new Date(Date.parse($(this).find('event\\:start, start').text()));
			var endDate = new Date(Date.parse($(this).find('event\\:end, end').text()));
	...
	var month=monthAsString(eventDate.getMonth());
	 ...
	 '<li><div class="item"><div class="date"><span class="day">'+day+'</span>'+
	 '<span class="month">'+month+'</span>'+
	 '<span class="num">'+dayOfMonth+'</span></div>'+
	 '<div class="content"><p><a href="'+$(this).find('link').text()+'">'+eventTitle+'</a></p></div></div></li>'
	);
     	}
    }
